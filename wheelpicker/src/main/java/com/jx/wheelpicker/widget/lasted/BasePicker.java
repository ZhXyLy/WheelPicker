 ...
 * @hide
 */
public class BasePicker extends View implements IBasePicker, Runnable {
     ...
     * 是否让文字自适应大小
     */
    private boolean mAdjustTextSize;
    ...
    public BasePicker(Context context, AttributeSet attrs) {
        ...
        for (String s : stringArray) {
            StringData stringData = new StringData(s);
    ...
    private void computeTextSize() {
        ...
        if (hasSameWidth) {
            ...
            mTextMaxWidth = (int) mPaint.measureText
                    (mData.get(mTextMaxWidthPosition).getText());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int drawnDataPos = drawnDataStartPos + mSelectedItemPosition,
            ...
            if (isCyclic) {
                ...
                name = mData.get(actualPos).getText();
            } else {
                if (isPosInRang(drawnDataPos)) {
                    name = mData.get(drawnDataPos).getText();
                }
            }
...
//            }
    ...
    private void refitText(String text, Paint paint) {
        float textSize = getItemTextSize();
        ...
        float textWidth = boundsRect.width();
...
//                }
    ...
    public void run() {
        ...
        if (mScroller.isFinished() && !isForceFinishScroll) {
            ...
            wheelScrollChanged(mData.get(position));
            if (null != mOnWheelChangeListener) {
    ...
    private void wheelScrollChanged(Data data) {
        if (onWheelScrollChangeListener != null) {
            onWheelScrollChangeListener.onWheelScroll(this, data);
        }
...
//        playSoundEffect(SoundEffectConstants.CLICK);
    ...
    public void setSelectedItemPosition(int position) {
        ...
        requestLayout();
        invalidate();
        if (mData.size()>mCurrentItemPosition) {
    ...
    public boolean isAdjustTextSize() {
        return mAdjustTextSize;
    }

    public void setAdjustTextSize(boolean adjustTextSize) {
        this.mAdjustTextSize = adjustTextSize;
        requestLayout();
        invalidate();
    }
    ...
    public interface OnItemSelectedListener {
         ...
         */
        void onItemSelected(BasePicker picker, Data data, int position);
    }
    ...
    public interface OnWheelScrollChangeListener {
         ...
         * 停止滚动即回调
         *
         * @param picker BasePicker
         * @param data   Data
         */
        void onWheelScroll(BasePicker picker, Data data);
    }
}